// Indian States & UTs with major cities. Ahmedabad lives under Gujarat.
window.LILY_INDIA = {
...
// ---- Centralized shipping config (update here if Porter rates change) ----
window.LILY_SHIPPING = {
  origin: { lat: 23.0225, lng: 72.5714 }, // Ahmedabad fulfillment
  freeState: "Gujarat",
  freeCity: "Ahmedabad",
  detour: 1.25,   // straight-line → road distance factor
  // Distance brackets (km upper-bound → fee ₹). Ordered ascending. Easy to swap for a live Porter quote later.
  brackets: [
    { maxKm: 25, fee: 0 },
    { maxKm: 75, fee: 40 },
    { maxKm: 150, fee: 70 },
...
// Approx coordinates: state centroid, with major-city overrides for accuracy.
window.LILY_GEO = {
  states: {
...
window.LILY_geoFor = (state, city) => (window.LILY_GEO.cities[city] || window.LILY_GEO.states[state] || window.LILY_SHIPPING.origin);
window.LILY_distanceKm = (state, city) => {
  const o = window.LILY_SHIPPING.origin, p = window.LILY_geoFor(state, city);
  ...
  return 2 * R * Math.asin(Math.sqrt(a)) * window.LILY_SHIPPING.detour;
};
window.LILY_shipFor = (state, city) => {
  const c = window.LILY_SHIPPING;
  if (state === c.freeState && city === c.freeCity) return 0;
  const km = window.LILY_distanceKm(state, city);
  const b = c.brackets.find(b => km <= b.maxKm) || c.brackets[c.brackets.length - 1];
  return b.fee;
};
